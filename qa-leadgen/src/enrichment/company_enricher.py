...
class CompanyEnricher:
    ...
    def _enrich_single(self, company_name: str, job: JobPosting) -> CompanyContact:
        ...
        if website:
            ...
            email, source = self._find_contact_on_site(website)
            if email:
        ...
        if not contact.general_contact_email and contact.website:
            ...
            contact.enrichment_source = "inferred HR email pattern"
    ...
    def _find_contact_on_site(self, website: str) -> tuple[str, str]:
        base = website.rstrip("/")
            ...
            try:
                ...
                for anchor in soup.select("a[href^='mailto:']"):
                    ...
                    if EMAIL_PATTERN.match(email):
                        return email, f"mailto link on {page_url}"

                page_text = soup.get_text(separator=" ")
                email = extract_email(page_text)
                if email and not self._is_generic_spam_trap(email):
                    return email, f"public page {page_url}"

                industry = self._extract_meta(soup, "industry")
                size = self._extract_meta(soup, "company size")
